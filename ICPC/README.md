...
- 本次作业提供部分样例数据，数据存放在本仓库 `data` 目录下。注意，这些数据并非 Online Judge 上的测试数据点，本地通过所有数据点并不能保证你能够通过 OJ 上的测试。

- 除本地提供的样例数据外，我们也鼓励根据自己的程序逻辑，尝试自己设计测试数据辅助调试。
